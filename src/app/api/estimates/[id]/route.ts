...
export async function GET(
  ...
  try {
    ...
    console.error('Error in GET estimate:', error)
    return NextResponse.json(
...
export async function PATCH(
  ...
  try {
    ...
    if (existingEstimate.created_by !== user.id && userProfile?.role !== 'manager') {
      return NextResponse.json(
    ...
    console.error('Error in PATCH estimate:', error)
    return NextResponse.json(
      { success: false, error: 'Internal server error' },
      { status: 500 }
    )
...
export async function DELETE(
  ...
  try {
    ...
    if (estimate.created_by !== user.id && userProfile?.role !== 'manager') {
      return NextResponse.json(
        { success: false, error: 'Insufficient permissions' },
        { status: 403 }
      )
    ...
    console.error('Error in DELETE estimate:', error)
    return NextResponse.json(
      { success: false, error: 'Internal server error' },
      { status: 500 }
    )
